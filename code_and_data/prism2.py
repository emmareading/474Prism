...
class Prism():
    ...
    def __build_rule(self, instances, attributes, label, cls):
        ...
        if cls == 'yes':
          while (best_pt != 1) and (len(self.__attr_not_in_R(avail_attr, R)) != 0): #until R is perfect or there are no attributes lefts
            for A in self.__attr_not_in_R(avail_attr, R): # for each attribute not in R
              for v in self.__get_attr_values(instances, A): # and each value v             
                pt_tuple = self.__rule_accuracy(clslist, A, v) # total instances of V for that class 
                v_total = self.__rule_accuracy(coverage, A, v) #total instances of V for all classes
                p = pt_tuple[1]
                v_p = v_total[1]
                pt = float(p)/v_p
                #initialize t
                ...
                if pt > best_pt:
                  best_pt, best_A, best_v = pt, A, v #best_t = t
                
                #also need to keep track of best in case of tie

            best = best + (best_A, best_v)
            #coverage = self.__apply_rule(coverage, best)
            R.append(best)
            best_A = 0 
            best_v = 0 
            best_pt = 0
            best_t = 0
            best = ()
          #change coverage to new coverage not including first rule ?
          #coverage = self.__apply_rule(coverage, best)
              
              #if we reset best_A, best_v, best_pt, best_t and best=() BELOW R.append HERE, then it combines 2 rules into 1
              #best = best + (best_A, best_v)
              #R.append(best)
             
            
            #below if statement works for first case, but its happening after all A's have been checked, need it to be added for each A
            #if best_pt == 1:
            #  best = best + (best_A, best_v)
            #  R.append(best)
          #else: the probability is not perfect, so go through with highest probability and find secondary goal
